...
public class ContactUsVrsaPage {
    ...
    // Contact Us is a Flutter screen and QA navigation can take a few seconds even after
    // UiAutomator2 has successfully delivered the tap. Keep this timeout local to this
    // navigation instead of changing global driver waits for the whole framework.
    private static final long CONTACT_US_NAVIGATION_TIMEOUT_MS = 10_000L;
    private static final long CONTACT_US_TRANSITION_GRACE_MS = 3_000L;
    private static final long CONTACT_US_RECOVERY_WAIT_MS = 4_000L;
    private static final long CONTACT_US_POLL_MS = 250L;
    ...
    private static final String TXT_POSTAL_ADDRESS =
    ...
    // Contact Us service dropdown. Appium Inspector exposes both the selected value and
    // the opened dropdown options through content-desc/accessibility id.
    private final By fundAdvisorService = AppiumBy.accessibilityId("Value Research Fund Advisor");
    private final By stockAdvisorService = AppiumBy.accessibilityId("Value Research Stock Advisor");
    ...
    public ContactUsVrsaPage(AndroidDriver driver) {
        this.driver = driver;
    }
    ...
    public void recoverContactUsIfNeeded() {
        try {
            ...
            if (isAdvisorAppForeground() && waitForContactUsPageVisible(CONTACT_US_RECOVERY_WAIT_MS)) {
    ...
    private void tapContactUsFromHubMoreSection() {
        ...
        // Never continue Hub gestures if the destination is already active. This protects against
        // delayed Flutter transitions where the tap succeeded but the first semantic frame arrived
        // slightly later than the click command returned.
        if (waitForContactUsPageVisible(500)) {
            ReportLogger.pass("Contact us page became active before another Hub gesture was required");
            return;
        }
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            // A successful tap may replace the Hub hierarchy between retries. Confirm state before
            // every swipe so we never scroll the Contact Us page while still calling it a Hub retry.
            if (waitForContactUsPageVisible(500)) {
                ReportLogger.pass("Contact us page detected before Hub scroll retry " + attempt);
                return;
            }

            if (!isHubAreaVisible()) {
                ReportLogger.debug(
                        "Hub content is no longer visible before Contact Us scroll retry " + attempt
                                + ". Waiting for destination transition instead of swiping."
                );

                if (waitForContactUsPageVisible(CONTACT_US_TRANSITION_GRACE_MS)) {
                    ReportLogger.pass("Contact us page detected during navigation transition");
                    return;
                }

                captureScreenshot("CU_Contact_Us_Unexpected_Navigation_State");
                throw new RuntimeException(
                        "Contact Us tap left the Hub, but the Contact us destination could not be verified. "
            ...
            if (waitForContactUsPageVisible(400)) {
                ReportLogger.pass("Contact us page detected after Hub scroll attempt " + attempt);
                return;
            }
        ...
        // Final destination check before reporting a Hub-tile failure. This catches a page that
        // completed its Flutter transition just after the last controlled attempt.
        if (waitForContactUsPageVisible(CONTACT_US_TRANSITION_GRACE_MS)) {
            ReportLogger.pass("Contact us page detected after final navigation grace period");
            return;
        }
    ...
    private boolean clickContactUsElement(WebElement element, String strategy) {
        ...
        // even when Inspector reports clickable=false. The element must not be reused after this
        // call because a successful navigation replaces the Hub hierarchy and makes it stale.
        if (tapWithMobileClickGesture(element, "Contact Us")) {
            if (waitForContactUsPageVisible(CONTACT_US_NAVIGATION_TIMEOUT_MS)) {
                ReportLogger.pass("Contact Us opened using element-based mobile: clickGesture");
                return true;
            }

            // If Hub has disappeared, do not touch the old WebElement or issue Hub gestures. Give
            // Flutter one final transition window to expose destination semantics.
            if (!isHubAreaVisible()) {
                ReportLogger.debug(
                        "Contact Us clickGesture removed Hub content, but destination semantics are not ready yet. "
                                + "Waiting without reusing the stale Hub element."
                );

                if (waitForContactUsPageVisible(CONTACT_US_TRANSITION_GRACE_MS)) {
                    ReportLogger.pass("Contact Us opened after delayed Flutter transition");
                    return true;
                }
            ...
            ReportLogger.debug(
                    "Contact Us clickGesture completed but Hub is still active after "
                            + CONTACT_US_NAVIGATION_TIMEOUT_MS + " ms. Trying one fresh-element fallback."
            );
        }

        // FALLBACK: only while Hub is confirmed active. Re-find the semantic node so we never call
        // getRect()/click on a cached element from the pre-navigation hierarchy.
        if (!isHubAreaVisible()) {
            if (waitForContactUsPageVisible(CONTACT_US_TRANSITION_GRACE_MS)) {
                ReportLogger.pass("Contact Us opened before fresh-element fallback was required");
                return true;
            }
            return false;
        }

        WebElement freshElement = findFirstDisplayedElement(contactUsHubTile);
        if (freshElement == null) {
            ReportLogger.debug(
                    "Fresh Contact Us semantic element was not available for fallback. "
                            + "Returning to controlled Hub discovery instead of tapping cached coordinates."
            );
            return false;
        }

        if (tapDynamicElementCenter(freshElement, "Contact Us fresh element-center fallback")) {
            if (waitForContactUsPageVisible(CONTACT_US_NAVIGATION_TIMEOUT_MS)) {
                ReportLogger.pass("Contact Us opened using fresh dynamic element-center fallback");
                return true;
            }
        }
    ...
    private boolean waitForContactUsPageVisible(long timeoutMillis) {
        ...
        do {
            // Reuse the complete screen detector instead of checking only title/Your Message.
            // This remains valid if Flutter exposes another strong Contact Us semantic marker first.
            if (isContactUsPageVisible()) {
                return true;
            }
    ...
    private void waitForContactUsPage() {
        ...
        if (!waitForContactUsPageVisible(CONTACT_US_NAVIGATION_TIMEOUT_MS)) {
            captureScreenshot("CU_Contact_Us_Page_Load_Timeout");
            throw new RuntimeException(
                    "Contact us page did not load within "
                            + CONTACT_US_NAVIGATION_TIMEOUT_MS
                            + " ms. Current package=" + safeCurrentPackage()
            );
        }
    ...
    public void verifyPostalAddress() {
        try {
            ...
            // Strict business validation. The previous test only checked generic fragments
            // (company, sector, Noida and PIN), so the incorrect short address
            // "Noida, 201301" could pass. These exact address-line fragments make CU_005
            // fail until Gautam Buddha Nagar and Uttar Pradesh are present.
            assertTextFragmentsPresentWithReport(
                    postalAddress,
    ...
    /**
     * CU_024
     * Select Value Research Stock Advisor and validate that Escalation Matrix opens
     * the Stock Advisor matrix target, not the Fund Advisor (VRFA) matrix.
     *
     * Important: Android Chrome's PDF viewer does not reliably expose rendered PDF body
     * text through UiAutomator2. Therefore this test validates the external browser target
     * (URL/address-bar/title/PDF filename) instead of waiting for PDF body text.
     */
    public void verifyStockAdvisorEscalationMatrixPdf() {
        try {
            ReportLogger.step(
                    "CU_024 - Selecting Value Research Stock Advisor and validating Escalation Matrix target"
            );
            ...
            selectStockAdvisorFromDropdown();

            if (!isStockAdvisorSelected()) {
                captureScreenshot("CU_024_Stock_Advisor_Selection_Not_Confirmed");
                throw new RuntimeException(
                        "Value Research Stock Advisor was not confirmed as the selected Contact Us service."
                );
            }

            ReportLogger.pass("Value Research Stock Advisor is selected in Contact Us dropdown");

            String beforePackage = safeCurrentPackage();
            tapRequiredWithDownScroll(
                    escalationMatrix,
                    "Escalation Matrix link for Value Research Stock Advisor",
                    3
            );

            if (!waitForExternalPdfNavigation(beforePackage, 6000)) {
                captureScreenshot("CU_024_Stock_Advisor_Matrix_Did_Not_Open");
                throw new RuntimeException(
                        "Escalation Matrix did not leave Contact Us or open an external PDF/browser after Stock Advisor selection."
                );
            }

            // Capture the external target immediately. Waiting for rendered PDF text is unreliable
            // because Chrome's Android PDF viewer usually does not expose PDF body text to UiAutomator2.
            String externalTarget = captureExternalBrowserTarget(5000);
            ReportLogger.step("Captured external browser target: " + cleanError(externalTarget));

            assertStockAdvisorMatrixTarget(externalTarget);

            ReportLogger.pass(
                    "CU_024 passed: Stock Advisor selection opened a Stock Advisor Escalation Matrix target"
            );
        } catch (Exception e) {
            captureScreenshot("CU_024_Stock_Advisor_Escalation_Matrix_PDF_Failure");
            throw new RuntimeException(
                    "Stock Advisor Escalation Matrix PDF validation failed: " + cleanError(e.getMessage()),
                    e
            );
        } finally {
            // CU_024 is intentionally followed by the final Back-to-Hub case. Restore Contact Us
            // here so the suite stays continuous and CU_023 can perform the final back navigation.
            try {
                returnToContactUsAfterExternalAction();
            } catch (Exception recoveryError) {
                ReportLogger.debug(
                        "CU_024 cleanup could not restore Contact Us: "
                                + cleanError(recoveryError.getMessage())
                );
            }
        }
    }

    private void selectStockAdvisorFromDropdown() {
        ReportLogger.step("Selecting Value Research Stock Advisor from Contact Us service dropdown");

        // Previous tests may leave Contact Us scrolled near the bottom. Bring the service dropdown
        // back into the visible viewport without using fixed coordinates.
        for (int attempt = 0; attempt <= 6; attempt++) {
            WebElement stock = findFirstDisplayedElement(stockAdvisorService);
            WebElement fund = findFirstDisplayedElement(fundAdvisorService);

            // Closed dropdown with Stock Advisor already selected.
            if (stock != null && fund == null) {
                ReportLogger.pass("Value Research Stock Advisor is already selected");
                return;
            }

            // Open dropdown: both options are visible. Select Stock Advisor directly.
            if (stock != null && fund != null) {
                if (!tapElementIfPresent(stockAdvisorService, "Value Research Stock Advisor dropdown option")) {
                    throw new RuntimeException("Stock Advisor option was visible but could not be tapped.");
                }

                if (waitForStockAdvisorSelection(2500)) {
                    ReportLogger.pass("Selected Value Research Stock Advisor from dropdown");
                    return;
                }

                throw new RuntimeException("Stock Advisor option was tapped but dropdown selection did not update.");
            }

            // Closed dropdown with Fund Advisor currently selected. Open it first.
            if (fund != null) {
                if (!tapElementIfPresent(fundAdvisorService, "Current Fund Advisor dropdown value")) {
                    throw new RuntimeException("Fund Advisor dropdown value was visible but could not be tapped.");
                }

                long optionDeadline = System.currentTimeMillis() + 2500;
                while (System.currentTimeMillis() < optionDeadline) {
                    if (findFirstDisplayedElement(stockAdvisorService) != null) {
                        break;
                    }
                    sleep(200);
                }
                ...
                if (findFirstDisplayedElement(stockAdvisorService) == null) {
                    throw new RuntimeException(
                            "Contact Us service dropdown opened, but Value Research Stock Advisor option was not displayed."
                    );
                }

                if (!tapElementIfPresent(stockAdvisorService, "Value Research Stock Advisor dropdown option")) {
                    throw new RuntimeException("Unable to tap Value Research Stock Advisor dropdown option.");
                }
                ...
                throw new RuntimeException("Stock Advisor selection was not retained after tapping the option.");
            }
        ...
        captureScreenshot("CU_024_Service_Dropdown_Not_Found");
        throw new RuntimeException(
                "Contact Us service dropdown was not found after scrolling to the top area."
        );
    }

    private boolean waitForStockAdvisorSelection(long timeoutMillis) {
        long deadline = System.currentTimeMillis() + timeoutMillis;
    ...
    private boolean isStockAdvisorSelected() {
        WebElement stock = findFirstDisplayedElement(stockAdvisorService);
        WebElement fund = findFirstDisplayedElement(fundAdvisorService);

        // In the closed dropdown Inspector exposes only the currently selected service.
        return stock != null && fund == null;
    }

    private boolean waitForExternalPdfNavigation(String beforePackage, long timeoutMillis) {
        long deadline = System.currentTimeMillis() + timeoutMillis;
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            boolean packageChanged = beforePackage != null
                    ...
                    && !beforePackage.equalsIgnoreCase(currentPackage);

            if (packageChanged || !isContactUsPageVisible()) {
                ReportLogger.pass(
                        "Escalation Matrix navigation detected"
    ...
    /**
     * Capture the browser/PDF target using Chrome native UI semantics.
     *
     * The PDF body itself is not a reliable UiAutomator2 surface. The address bar, page title,
     * document filename, and other Chrome accessibility attributes are much more stable.
     */
    private String captureExternalBrowserTarget(long timeoutMillis) {
        long deadline = System.currentTimeMillis() + timeoutMillis;
        String lastPackage = "";
        String lastEvidence = "";
        int lastSourceLength = 0;

        while (System.currentTimeMillis() < deadline) {
            lastPackage = safeCurrentPackage();

            if (ADVISOR_APP_PACKAGE.equalsIgnoreCase(lastPackage)) {
                sleep(200);
                continue;
            }

            StringBuilder evidence = new StringBuilder();
            appendBrowserElementEvidence(
                    evidence,
            ...
            appendBrowserElementEvidence(
                    evidence,
                    AppiumBy.id("com.android.chrome:id/title_bar"),
                    "title_bar"
            );

            // Fallback: extract only useful values from the external hierarchy. This can expose the
            // PDF filename/title even when the Chrome URL field itself is temporarily hidden.
            String source = safePageSource();
            lastSourceLength = source.length();
            String relevantSourceValues = extractRelevantExternalValues(source);
            if (!relevantSourceValues.isEmpty()) {
                if (evidence.length() > 0) {
                    evidence.append(" | ");
                }
                evidence.append("hierarchy=").append(relevantSourceValues);
            }

            lastEvidence = evidence.toString();
            String normalized = normalizeForSearch(lastEvidence);
            ...
            if (containsAnyNormalized(
                ...
                return "package=" + lastPackage + " | " + lastEvidence;
            }
        ...
        if (!lastEvidence.isEmpty()) {
            return "package=" + lastPackage + " | " + lastEvidence
                    + " | hierarchyLength=" + lastSourceLength;
        }

        return "package=" + lastPackage
                + " | no URL/title/PDF filename exposed by Chrome"
                + " | hierarchyLength=" + lastSourceLength;
    }

    private void appendBrowserElementEvidence(StringBuilder out, By locator, String label) {
        try {
            ...
            for (WebElement element : elements) {
                ...
                appendEvidenceValue(values, safeVisibleText(element));
                appendEvidenceValue(values, safeAttribute(element, "text"));
                appendEvidenceValue(values, safeAttribute(element, "content-desc"));
                appendEvidenceValue(values, safeAttribute(element, "contentDescription"));
                appendEvidenceValue(values, safeAttribute(element, "name"));
                appendEvidenceValue(values, safeAttribute(element, "value"));

                String cleaned = values.toString().trim();
                if (!cleaned.isEmpty()) {
                    if (out.length() > 0) {
                        out.append(" | ");
                    }
                    out.append(label).append("=").append(cleaned);
                }
            }
        } catch (Exception e) {
            ReportLogger.debug(
                    "Unable to read Chrome " + label + ": " + cleanError(e.getMessage())
            );
        }
    }

    private String safeVisibleText(WebElement element) {
        try {
    ...
    private void appendEvidenceValue(StringBuilder out, String value) {
        if (value == null) {
            return;
        }
    ...
    private String extractRelevantExternalValues(String source) {
        ...
        java.util.regex.Pattern pattern = java.util.regex.Pattern.compile(
                "(?:text|content-desc|name|value)=\\\"([^\\\"]+)\\\"",
                java.util.regex.Pattern.CASE_INSENSITIVE
        );
        java.util.regex.Matcher matcher = pattern.matcher(source);

        while (matcher.find() && values.size() < 16) {
            String value = decodeXmlEntities(matcher.group(1));
    ...
    private String decodeUrlForValidation(String value) {
        if (value == null) {
            return "";
        }
        ...
        for (int attempt = 0; attempt < 2; attempt++) {
            try {
                String next = java.net.URLDecoder.decode(
                        decoded,
    ...
    private boolean containsAnyNormalized(String normalizedValue, String... markers) {
        if (normalizedValue == null || markers == null) {
            return false;
        }

        for (String marker : markers) {
            if (marker != null && normalizedValue.contains(normalizeForSearch(marker))) {
                return true;
            }
        }
    ...
    private void assertStockAdvisorMatrixTarget(String externalTarget) {
        String decodedTarget = decodeUrlForValidation(externalTarget);
        String normalized = normalizeForSearch(decodedTarget);

        boolean stockMarker = containsAnyNormalized(
                normalized,
        ...
        boolean fundMarker = containsAnyNormalized(
                normalized,
                "value research fund advisor",
                "fund advisor",
                "fund-advisor",
                "fund_advisor",
                "fundadvisor",
        ...
        boolean documentMarker = containsAnyNormalized(
                normalized,
                ".pdf",
                "matrix",
                "grievance",
        ...
        if (fundMarker && !stockMarker) {
            captureScreenshot("CU_024_WRONG_VRFA_PDF_OPENED_FOR_STOCK_ADVISOR");
            ReportLogger.fail(
                    "Wrong Escalation Matrix target detected: Fund Advisor/VRFA target opened for Stock Advisor selection"
            );
            throw new RuntimeException(
                    "Wrong PDF opened. Expected Value Research Stock Advisor (VRSA) Escalation Matrix, "
                            + "but a Value Research Fund Advisor (VRFA) target was detected. Actual target: "
                            + cleanError(decodedTarget)
            );
        }

        if (stockMarker && documentMarker) {
            ReportLogger.pass(
                    "Validated Stock Advisor Escalation Matrix browser target: " + cleanError(decodedTarget)
            );
            return;
        }

        if (stockMarker) {
            // Some Chrome builds expose only the document title and not the full URL/.pdf suffix.
            ReportLogger.pass(
                    "Validated Stock Advisor marker in external browser target: " + cleanError(decodedTarget)
            );
            return;
        }

        captureScreenshot("CU_024_EXTERNAL_TARGET_COULD_NOT_BE_CLASSIFIED");
        throw new RuntimeException(
                "External PDF/browser opened, but the target could not be classified as VRSA or VRFA. "
                        + "Captured target: " + cleanError(decodedTarget)
        );
    }
